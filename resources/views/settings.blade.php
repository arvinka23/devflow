...
<div class="max-w-3xl space-y-8">
    ...
    <div class="bg-card rounded-2xl border border-border">
        ...
        <div class="p-6 flex items-center gap-6">
            <!-- Clickable avatar -->
            <label for="avatar-input" class="relative cursor-pointer group/av shrink-0">
        ...
        <!-- Hidden upload form -->
        <form id="avatar-form" action="{{ route('settings.avatar') }}" method="POST" enctype="multipart/form-data" class="hidden">
            @csrf
            <input type="file" name="avatar" id="avatar-file-proxy">
        </form>
        <input type="file" id="avatar-input" accept="image/*" class="hidden"
               onchange="proxyAvatarUpload(this)">
    </div>
...
@push('scripts')
<script>
function proxyAvatarUpload(input) {
    if (!input.files || !input.files[0]) return;
    const proxy = document.getElementById('avatar-file-proxy');
